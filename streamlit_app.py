...
def generate_sparkline(score):
    ...
    for i in range(20):
        if i < bars:
            bar_html += f'<div style="width: 8px; height: {20 + (i * 2)}px; background: linear-gradient(0deg, #667eea, #764ba2); display: inline-block; margin: 0 2px; border-radius: 4px;"></div>'
        else:
...
def safety_confidence(risk):
    if risk == "low":
        return 95, "🟢 Low Risk – Standard safety constraints applied", "#4CAF50"
    return 45, "🔴 High Risk – Enhanced safety & ethics validation", "#f44336"
...
if generate_btn:
    ...
    with col2:
        ...
        with c1:
            ...
            st.markdown(f'<p style="color: #666; margin: 0;">Risk</p>', unsafe_allow_html=True)
            st.markdown(f'<p class="metric-value">{risk_color}</p>', unsafe_allow_html=True)
            st.markdown(f'<p style="color: #666; font-size: 14px;">{plan["risk"].upper()}</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        
        with c2:
            st.markdown('<div class="metric-card">', unsafe_allow_html=True)
            st.markdown(f'<p style="color: #666; margin: 0;">Quality</p>', unsafe_allow_html=True)
            st.markdown(f'<p class="metric-value">{evaluation["quality_score"]}</p>', unsafe_allow_html=True)
            st.markdown(f'<p style="color: #666; font-size: 14px;">/3</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        
        with c3:
            st.markdown('<div class="metric-card">', unsafe_allow_html=True)
            verdict_color = "🟢" if evaluation["verdict"] == "high_quality" else "🟡"
            st.markdown(f'<p style="color: #666; margin: 0;">Verdict</p>', unsafe_allow_html=True)
            st.markdown(f'<p class="metric-value">{verdict_color}</p>', unsafe_allow_html=True)
            st.markdown(f'<p style="color: #666; font-size: 14px;">{evaluation["verdict"].replace("_", " ").title()}</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 10px;">
            <span style="color: #666;">Subtle</span>
            <span style="color: #666;">Moderate</span>
            <span style="color: #666;">Exaggerated</span>
        </div>
        """, unsafe_allow_html=True)
